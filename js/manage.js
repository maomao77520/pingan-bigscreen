...
function initLines() {
    ...
    let incomeOptions = getOptions({
        ...
        lineColor: '#FFCC00',
        data: totalData.lineData.income
    });
    ...
    let averageOptions = getOptions({
        ...
        lineColor: '#4CD964',
        data: totalData.lineData.average
    });
...
function initPie() {
    ...
    incomePie.setOption(getPieOptions({data: totalData.pieData.income}));
    let orderPie = echarts.init(document.getElementById('J_order_pie'));
    orderPie.setOption(getPieOptions({data: totalData.pieData.order}));
}
...
function initBar() {
    let option = {
        ...
        xAxis: {
            type: 'category',
            data: totalData.barData.income.category,
            axisLabel: {
                show: true,
                fontSize: 12,
                color: '#93B0EB'
        ...
        series: [{
            ...
            data: totalData.barData.income.data
        }]
    };
    ...
    $('#J_bar_tabs a').on('click', function() {
        ...
        $(this).addClass('active');
        let name = $(this).data('name');
        option.xAxis.data = totalData.barData[name].category;
        option.yAxis.name = mapper[name];
        option.series[0].data = totalData.barData[name].data;
        bar.setOption(option);
    });
}
...
function initList() {
    ...
    $('#J_list_tabs a').on('click', function() {
        $('#J_list_tabs a').removeClass('active');
        $(this).addClass('active');
        let name = $(this).data('name');
        data = totalData.listData[name];
        $('#J_list_wrap').html(tmp(data));
    });
